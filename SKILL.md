...
description: |
  v1.0 정책기획 풀 파이프라인 (Phase 0~3). 리서치프레임→축별리서치→스파인→기획안→제출청소까지 일관 수행.
    P1: 정책기획, policy planning, 정책기획스킬, 정책안, 공공기획, 캠프전략, 캠페인전략, 선거전략, 정책제안, 리서치프레임, 스파인도출, 기획안, 제출청소, 정책리서치, 정책설계, 정책문서, 정책보고서, 정책입안, 공약기획, 정책분석.
...
| 2 | **축 헤드 단언형** | 각 축 헤드 ≤20자. 결론·주장형 | 라벨식·질문식 헤드 |
| 3 | **수치+출처 강제** | 모든 정책 주장 = 수치 + 1차 출처 1조 | "큰 효과가 있을" 추상 |
| 4 | **문장당 1명제** | 본문 1행 = 1주장. AND·OR 묶기 ✗ | "A하면서 B도 하고" |
| 5 | **AI식 사전회피** | "관련 부서와 긴밀히 협력하여", "지속 가능한 발전을", "포용적 성장" — 작성 단계 차단 | 행정관용어·추상명사 |

### Phase별 PRE_WRITE

- **Phase 1 스파인 도출**: 스파인 1줄 ≤25자 강제. 형 확인 게이트 통과 조건
- **Phase 2 기획안 작성**: 축 헤드·소제목·본문 5룰 강제
- **Phase 3 제출청소**: 5룰 위반 grep = 청소 1차 필터
